...
<template>
  <div>
    <div v-if="showHero && article" class="bg-navy pb-6">
      <PageContainer class="pt-5 sm:pt-6">
        <div class="flex items-center justify-between">
          <NuxtLink to="/" aria-label="Back to all articles" class="text-white transition-colors hover:text-white/70">
          ...
          <button type="button" class="flex items-center gap-0.5 text-white transition-colors hover:text-white/70"
            :aria-pressed="isSaved" aria-label="Save article" @click="isSaved = !isSaved">
            <svg class="h-5 w-5" viewBox="0 0 24 24" :fill="isSaved ? 'currentColor' : 'none'" stroke="currentColor"
      ...
      <PageContainer>
        <ArticleImage :src="article.imageUrl" :alt="article.title" priority aspect-class="aspect-[16/9]"
          class="rounded-2xl" />
      </PageContainer>
    </div>
    ...
    <PageContainer class="py-8 sm:py-10">
      ...
      <ErrorState v-else-if="isError" :message="errorMessage ?? 'Something went wrong while loading this article.'"
        :kind="errorKind" @retry="retry" />

      <EmptyState v-else-if="!article" title="Article not found"
